...
public class RestApiTextDecoder extends TextDecoder {
    ...
    public RestApiTextDecoder(DefaultConnection connection, TextEncoder encoder, boolean rootDecoder) {
        super(connection, encoder, createFilter(connection), TEXT_PARSERS, rootDecoder);
    }

    private static RestApiFilter createFilter(DefaultConnection connection) {
        IOService ioService = connection.getEndpoint().getNetworkingService().getIoService();
